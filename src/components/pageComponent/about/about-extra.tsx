...
const AboutExtra = () => (
  <section className="px-6 py-8">
    {/* Hobbies Section */}
    <div className="max-w-2xl border border-zinc-900 border-dashed bg-black/10 overflow-hidden">
      ...
      <div className="divide-y divide-zinc-900/50">
        {hobbies.map((hobby, index) => (
          ...
          >
            ...
            <div className="absolute left-0 top-0 bottom-0 w-[2px] bg-white/40 opacity-0 group-hover:opacity-100 transition-opacity" />

            <div className="flex items-start gap-4">
              <span className="font-mono text-[9px] text-zinc-700 mt-0.5 shrink-0">
                {String(index + 1).padStart(2, "0")}
              </span>
              <div className="flex-1">
                <h4 className="font-mono text-sm text-zinc-400 group-hover:text-white transition-colors">
                  {hobby.name}
                </h4>
                <p className="font-mono text-[11px] text-zinc-600 group-hover:text-zinc-500 mt-0.5 leading-relaxed transition-colors">
                  {hobby.desc}
                </p>
              </div>
            </div>
