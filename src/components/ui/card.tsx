...
function DotsPattern({
  ...
  const sharedClasses =
    "rounded-full outline outline-8 dark:outline-zinc-950 sm:my-6 md:my-8 size-1 my-4 outline-zinc-50 bg-green-400";
    ...
    <>
      ...
      <div className="relative w-full border-zinc-400 border-x dark:border-zinc-700">
        ...
        <div className="relative z-20 mx-auto py-8">
          <CardContent>
            {title && (
              <h3 className="mb-1 font-bold text-zinc-900 text-lg dark:text-zinc-100">
                {title}
              </h3>
            )}
            {description && (
  ...
  if (variant === "dots") {
    return (
      <div
        ...
        {...props}
      >
        <DotsPattern title={title} description={description}>
          {children}
        </DotsPattern>
      </div>
    );
